-- Q: Can we come up with better urine output and serum creatinine criteria and stages for acute kidney injury in the ICU based on clinical outcomes?
-- create Team8 needing tables (please run each part separately)
-- By xiaoli liu (2020.02.14 - 2.15)
-- Part 1. demographic information: save to bigquery team8 (final_cohort_demographic_info)
-- Part 2. urine output initial information: save to bigquery team8 (final_cohort_urine_info)
-- Part 3. creatinine initial information: save to bigquery team8 (final_cohort_creatinine_info)
-- Part 4. develop four decision tree models - using data: save to bigquery team8 (final_creatinine_uo_treemodel_results)
-- Part 5. kidgo - predict results (roc)： save to bigquery team8 (kidgo_roc_results)
...
-- drop the patients receiving dialysis before ICU admission
, rrt_info_0 as (
...
-- drop: the patients didn't exist records less than 2 times
, urine_cohort_0 as (
...
-- drop: the patients didn't exist records less than 2 times (the condition is different from urine output) 
...
-- get the integrated information 
select oi.*
...
-- using the unit of hour to record the total urine output of an hour
, urine_info as (
	select icustay_id, los_icu_hour, sum(urineout) as urineout
	from urine_info_0
	group by icustay_id, los_icu_hour
...
-- using the unit of hour to record the creatinine
, creatinine_info as (
	select icustay_id, los_icu_hour, avg(creatinine) as creatinine
...
-- (creatinine max value; creatinine/creatinine baseline - ratio; increase creatinine; urine output)
...
-- due to the limited time, I can't solve it in short time using a function. So, the urine output was
